...
int main(int argc, char **argv)
{
	...
	try {
		...
		vector<double> u0(t.numEdgeShapes(), 0.1);
                vector<double> df(t.numEdgeShapes(), 0.2); 
		data.c->setU0(u0);
		data.c->setDform(df);
		data.c->setDU(du);
	...
	// Tile scale - use a factor on the initial scale found
	ub(3+t.numParameters()+2*t.numEdgeShapes()) = 1.5*x_1[x_1.size()-1]; 
